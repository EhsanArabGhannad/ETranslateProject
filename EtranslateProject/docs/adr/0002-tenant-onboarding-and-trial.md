...
- Identity publishes `TenantCreatedV1` through a PostgreSQL transactional outbox.
...
- PostgreSQL databases are separate Aspire resources (`identitydb` and `billingdb`).
- RabbitMQ is the local event transport.
...
- Local end-to-end development requires a supported Docker or Podman container runtime.
