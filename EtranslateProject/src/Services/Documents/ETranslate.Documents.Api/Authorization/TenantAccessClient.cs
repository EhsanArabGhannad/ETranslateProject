...
public sealed class TenantAccessClient(HttpClient httpClient)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        using var request = new HttpRequestMessage(HttpMethod.Get, $"/api/v1/tenants/{tenantId}/access");
        ...
        using (response)
        {
            ...
            return access is null
                ? TenantAccessResult.Unavailable()
                : TenantAccessResult.Authorized(new TenantActor(access.UserId, access.Role));
        }
    }

    private sealed record TenantAccessDto(Guid TenantId, Guid UserId, string TenantType, string Role);
}
...
public sealed record TenantAccessResult(TenantAccessStatus Status, TenantActor? Actor)
{
